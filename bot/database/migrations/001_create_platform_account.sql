...
-- 支持多账户绑定 (一个 Customer ID 可绑定多个平台账户)
-- ============================================
...
CREATE TABLE `yoshop_platform_account` (
  ...
  KEY `idx_customer_id` (`customer_id`,`wxapp_id`) USING BTREE COMMENT 'Customer ID 索引 (允许多个账户)',
  UNIQUE KEY `uk_user_platform` (`user_id`,`platform_type`,`customer_id`,`wxapp_id`) USING BTREE COMMENT '每个用户对每个平台的每个 Customer ID 只能绑定一次',
  KEY `idx_user_id` (`user_id`) USING BTREE,
  KEY `idx_platform_type` (`platform_type`) USING BTREE,
  KEY `idx_wxapp_id` (`wxapp_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='平台账户绑定表 (FB/IG Bot，支持多账户)';
...
-- (2, 'FACEBOOK', 'CUST_123456', 'Jane S***', 1, 10001);  -- 同一 Customer ID 绑定给另一个用户
