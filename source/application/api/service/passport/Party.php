...
class Party extends Basics
{
     ...
     */
    public function createUserOauth(int $userId, array $partyData = [])
    {
        try {
            // 获取oauthId和unionId
            if ($partyData['oauth_id']){
                $oauthInfo['oauth_id'] = $partyData['oauth_id'];
            }else{
        ...
        if (empty($oauthId)) {
            return (new UserOauthModel)->add([
                'user_id' => $userId,
                'oauth_type' => $partyData['oauth']??'ZALO',
    ...
    {
        $partyUserInfo = $partyData['userInfo']??$partyData;
        $data = [
            'nickName' => $partyUserInfo['nickName']??'',
            'gender' => $partyUserInfo['gender']??0
        ];
        // 下载用户头像
        if ($isGetAvatarUrl) {
            $data['avatarUrl'] = $partyData['avatarUrl'];
        }
        return $data;
    }
